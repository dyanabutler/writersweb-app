...
export async function getAllCharacters(storyId?: string): Promise<Character[]> {
  try {
    const query = supabase.from("characters").select("*")
    
    const { data, error } = await (storyId 
      ? query.eq("story_id", storyId).order("name", { ascending: true })
...
export async function getCharacterBySlug(slug: string, storyId?: string): Promise<Character | null> {
  try {
    const query = supabase.from("characters").select("*").eq("slug", slug)
    
    const { data, error } = await (storyId 
      ? query.eq("story_id", storyId).single()
      : query.single()
    )
...
export async function deleteCharacter(slug: string, storyId?: string): Promise<boolean> {
  try {
    let query = supabase.from("characters").delete().eq("slug", slug)
...
export async function getCharactersByLocation(location: string, storyId?: string): Promise<Character[]> {
  try {
    let query = supabase.from("characters").select("*").eq("location", location)
    ...
    console.error("Error in getCharactersByLocation:", error)
    return []
  }
}

export async function getCharactersByStatus(status: Character["status"], storyId?: string): Promise<Character[]> {
  try {
    let query = supabase.from("characters").select("*").eq("status", status)
